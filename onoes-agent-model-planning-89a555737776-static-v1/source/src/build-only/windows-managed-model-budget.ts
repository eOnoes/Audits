...
export class SqliteManagedModelBudget {
  ...
  snapshot() {
    return this.tx(() => {
      ...
      return deepFreeze({ budgetDigest: this.expectedBudgetDigest, currency: state.policy.currency,
  ...
  recordOutcome(terminalWire: unknown) {
    ...
    return this.tx(() => {
      ...
      this.db.prepare("UPDATE model_allocations SET terminal_json=? WHERE request_digest=? AND terminal_json IS NULL").run(canonicalJson(terminal), input.requestDigest);
      return this.scan().records.find(r => r.reservation.requestDigest === input.requestDigest)!;
    }, true);
  }
}
