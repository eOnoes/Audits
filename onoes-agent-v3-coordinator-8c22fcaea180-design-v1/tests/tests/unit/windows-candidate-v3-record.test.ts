...
test("v3 pre-outcome records reject supplied settlement or checkpoint context independently", () => {
  const f = fixture();
  for (let n = 0; n <= f.outcomeIndex; n++) {
    const input = wire({ ...f.record, events: f.record.events.slice(0, n) });
    assert.ok(parseCandidateV3Record(input, wire(metadata), null, null));
    for (const [core, a] of [[wire(f.core), null], [null, wire(f.checkpointA)], [wire(f.core), wire(f.checkpointA)]]) {
      denied(() => parseCandidateV3Record(input, wire(metadata), core, a));
    }
  }
});

test("v3 reservation itself must precede expiry even without any events", () => {
  const f = fixture(); f.record.events = [];
  f.record.reservedAt = time(9); assert.ok(parse(f, 0));
  for (const at of [time(10), time(11)]) {
    f.record.reservedAt = at; denied(() => parse(f, 0));
  }
});
