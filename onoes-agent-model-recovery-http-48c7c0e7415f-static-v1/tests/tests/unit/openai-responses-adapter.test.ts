...
test("generic retry policy honors non-retryable HTTP classifications", async () => {
  for (const status of [402, 405, 409, 410, 422]) {
    ...
    await assert.rejects(adapter.complete(request()), (error: unknown) =>
      error instanceof ProviderResponseError && error.code === "provider-unavailable"
      && error.retryable === false && error.retryCount === 0);
...
test("stream completion rejects subsequent payloads without emitting or retrying them", async () => {
  for (const suffix of [
    'data: {"type":"response.output_text.delta","delta":"late"}\n\n',
    'data: {"type":"response.completed","response":{"model":"other","usage":{}}}\n\n',
    'data: {"type":"unknown"}\n\n',
    'data: {"type":"response.output_text.delta","delta":"late"}',
  ]) for (const split of [false, true]) {
    let calls = 0, sleeps = 0;
    const emitted: string[] = [];
    const chunks = split ? [validSse(), suffix] : [validSse() + suffix];
    ...
    const adapter = new OpenAIResponsesAdapter({ apiKey: API_KEY, maxAttempts: 3,
      fetch: async () => { calls++; return response; }, sleep: async () => { sleeps++; } });
    await assert.rejects(adapter.complete(request(delta => emitted.push(delta))), providerCode("provider-malformed-response"));
...
test("stream completion permits bounded empty framing comments and DONE trailer", async () => {
  const adapter = new OpenAIResponsesAdapter({ apiKey: API_KEY,
    fetch: async () => new Response(validSse() + ': heartbeat\n\ndata: [DONE]\n\n') });
  assert.equal((await adapter.complete(request())).text, "ok");
});
