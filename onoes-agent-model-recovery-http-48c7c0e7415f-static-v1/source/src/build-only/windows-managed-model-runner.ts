...
type Reason = "configuration-invalid" | "confirmation-invalid" | "request-unavailable" | "request-used"
  | "budget-exceeded" | "busy" | "poisoned" | "closed" | "cancelled" | "timeout" | "unsettled-model"
  | "model-failed" | "response-invalid" | "allocation-unavailable" | "allocation-existing";
/** Local observations for locating a retained allocation, never a receipt,
 * remote-status proof, source response or authority to retry. */
export interface ManagedModelFailureRecovery {
  readonly schemaVersion: "agent-model-failure-recovery/v1";
  readonly kind: "local-attempt-observations-not-durable-receipt";
  readonly requestDigest: string;
  readonly profileDigest: string;
  readonly budgetDigest: string;
  readonly reservationAttempted: boolean;
  readonly reservationReadBackConfirmed: boolean;
  readonly providerCallbackStarted: boolean;
  readonly outcomeWriteAttempted: boolean;
  readonly outcomeReadBack: "response-observed" | "outcome-unknown" | null;
  readonly primaryReason: Reason;
  readonly automaticRetryAvailable: false;
  readonly remoteWorkStatus: "not-established";
  readonly approvalAvailable: false;
  readonly authority: "none";
}
export class ManagedModelRunnerError extends Error {
  constructor(readonly reason: Reason, readonly recovery: ManagedModelFailureRecovery | null = null) {
    super(`managed-model-runner-${reason}`); this.name = "ManagedModelRunnerError";
    Object.defineProperty(this, "recovery", { value: recovery === null ? null : deepFreeze({ ...recovery }),
...
export class WindowsManagedModelRunner {
  ...
  async run(handle: unknown, requestDigest: unknown, confirmationWire: unknown,
    ...
    let reservationAttempted = false, reservationReadBackConfirmed = false, outcomeWriteAttempted = false;
    let outcomeReadBack: ManagedModelFailureRecovery["outcomeReadBack"] = null;
    const failure = (reason: Reason, primaryReason: Reason) => new ManagedModelRunnerError(reason, {
      schemaVersion: "agent-model-failure-recovery/v1", kind: "local-attempt-observations-not-durable-receipt",
      requestDigest: request.requestDigest, profileDigest: this.#profile.profileDigest,
      budgetDigest: this.#profile.allocation.budgetDigest, reservationAttempted, reservationReadBackConfirmed,
      providerCallbackStarted: providerStarted, outcomeWriteAttempted, outcomeReadBack, primaryReason,
      automaticRetryAvailable: false, remoteWorkStatus: "not-established", approvalAvailable: false, authority: "none",
    });
    ...
    const done: Promise<Done> = Promise.resolve().then(() => {
      ...
      try {
        ...
        reservationReadBackConfirmed = true;
      } catch (error) {
    ...
    try {
      ...
      const primaryReason = error instanceof ManagedModelRunnerError ? error.reason : "response-invalid";
      // Finish the existing conservative bookkeeping before freezing observations.
      // A failed write/read-back still takes precedence, but cannot erase the
      // original bounded reason. Never issue a second uncertain settlement.
      try { if (reservation && !settlementAttempted) recordOutcome("outcome-unknown"); }
      catch { throw failure("allocation-unavailable", primaryReason); }
      throw failure(primaryReason, primaryReason);
    } finally {
