...
for (const same of [true, false]) test(`two independent worker connections start together for ${same ? "identical approval" : "same workspace"}: outcome consistency, not proof of contention`, async () => {
  const f = fixture(), workers: Worker[] = []; try {
    const first = intent(f.namespaceId, f.storeId), barrier = new SharedArrayBuffer(4);
    let ready = 0;
    const promises = [first, same ? first : intent(f.namespaceId, f.storeId)].map(input => new Promise<string>((resolveResult, reject) => {
