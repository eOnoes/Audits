...
export function initializeCandidateEffectLedger(db: Database.Database, namespaceId: string): string {
  return guard(() => {
    ...
    db.transaction(() => {
      ...
      db.prepare("INSERT INTO candidate_effect_meta VALUES (1,1,?,?)").run(storeId, namespaceId); schema(db);
    }).immediate();
    return storeId;
  });
}
...
const subjectFields = ["namespaceId", "workflowId", "workspaceDigest", "policyBindingDigest", "candidateDigest", "reviewMaterialDigest",
...
export class SqliteCandidateEffectLedger {
  #writePoisoned = false;
  constructor(private readonly db: Database.Database, private readonly storeId: string,
    private readonly namespaceId: string, private readonly now: () => string = () => new Date().toISOString()) {
  ...
  private preflight(): void {
    ...
    if (canonicalJson(rows) !== canonicalJson([{ singleton: 1, version: 1, store_id: this.storeId, namespace_id: this.namespaceId }])) effectFail("identity-mismatch");
  }
  private scan(): CandidateEffectRecord[] {
    ...
    const records = rows.map(row => {
      ...
      if (record.intent.namespaceId !== this.namespaceId || record.intent.operationId !== row.operation_id
  ...
  private readAll(): CandidateEffectRecord[] {
  ...
  read(operationId: string): CandidateEffectRecord | undefined {
    effectParse(effectUuid, operationId); return this.readAll().find(r => r.intent.operationId === operationId);
  }
  listBlocked(): readonly CandidateEffectRecord[] { return deepFreeze(this.readAll().filter(candidateEffectBlocked)); }
  private save(record: CandidateEffectRecord, insert: boolean): void {
  ...
  private write(operation: (records: CandidateEffectRecord[], at: string) => { record: CandidateEffectRecord; replayed: boolean }) {
    ...
    return guard(() => {
      ...
      try {
        ...
        const found = this.read(saved.record.intent.operationId);
  ...
  reserve(value: unknown) {
    ...
    if (intent.namespaceId !== this.namespaceId) effectFail("identity-mismatch");
