...
export class SyntheticPairWorld {
  ...
  counts = { bootstraps: 0, worker: 0, transactions: 0, commits: 0, readBacks: 0, anchorAttempts: 0, anchorAppends: 0, promotions: 0 };
  ...
  retire() { if (this.activity !== 0) return deny(); this.ownerHeld = false; this.epoch = uuid(Number.parseInt(this.epoch.slice(-12), 16) + 1); }
}
...
export class SyntheticPairCoordinator {
  ...
  constructor(readonly world: SyntheticPairWorld) {
    if (world.ownerHeld || world.activity || !world.fence || !world.schemaValid) throw new ModelDenied();
    const s = world.local, pair = requireEqualSyntheticPair(s.metadata, s.inventory, s.history, world.anchor);
    this.#s = Object.freeze({ ...s }); this.#identity = [...pair.identity]; this.#epoch = world.epoch;
    ...
    const request = bounded(wire({ domain: "synthetic-v3-bootstrap-request/v1", operation: "bootstrap", ...common,
      metadataWire: s.metadata, inventoryWire: s.inventory, historyWire: s.history }));
  ...
  async append(inventoryWire: unknown, checkpointWire: unknown, signal: AbortSignal): Promise<void> {
    ...
    try {
      ...
      const requestWire = bounded(wire({ domain: "synthetic-v3-append-request/v1", operation: "append", lifetimeId: this.#lifetime,
        requestNumber: ++this.#requestNumber, nonce: uuid(this.#nonce++), epoch: this.#epoch,
        pre, inputDigest: rawDigest(inventory), checkpointDigest: rawDigest(cp), deadline, inventoryWire: inventory, checkpointWire: cp }));
      w.cut("captured");
      this.#live(deadline, true); if (signal.aborted) return deny();
