...
function copyAdapterBytes(value: unknown, maximumBytes: number): Buffer {
  try {
    if (typeof value !== "object" || value === null || types.isProxy(value) || !types.isUint8Array(value)
      || !Number.isSafeInteger(maximumBytes) || maximumBytes < 0) throw new StepError("io-failed");
    // A trusted adapter's returned view can still shadow ordinary properties.
    // Check intrinsic storage before copying; never invoke its accessors or
    // subclass behavior. This bounds our copy, not allocations inside the adapter.
    ...
    if (types.isSharedArrayBuffer(backing) || length > maximumBytes) throw new StepError("io-failed");
    return Buffer.from(new Uint8Array(backing, offset, length));
  } catch { throw new StepError("io-failed"); }
}
...
export class WindowsManagedBuilderExecutor {
  ...
  async execute(frameInput: Uint8Array, signal: AbortSignal = new AbortController().signal): Promise<ManagedExecutionResult> {
    ...
    this.#busy = true;
    let activeIo: ManagedWorkspaceCustodySession | undefined, abandoned = false;
    ...
    try {
      ...
      await run(async s => {
        const opened = await this.#options.io.openCustody(s);
        // The bounded wait can finish before a cancellation-ignoring adapter
        // returns its holder. Retain cleanup in this observed callback: a late
        // holder must never be assigned to an already-finished execution or
        // escape close(). Rejection/pending close remains unconfirmed; neither
        // late success nor failure clears the executor's permanent poison.
        if (abandoned) { await opened.close(); throw new StepError("unsettled-work", false); }
        activeIo = opened;
      });
  ...
  private async step<T>(fn: (signal: AbortSignal) => Promise<T>, external: AbortSignal | undefined, availableMs: number): Promise<T> {
    ...
    try {
      ...
      if (first.type === "value") {
        ...
        // Selection by Promise.race precedes this await continuation. Abort can
        // arrive between them; the work is settled, but its value must not be
        // accepted as another forward step after cancellation.
        if (external?.aborted) throw new StepError("cancelled");
        return first.value;
      }
