...
export class SqliteWindowsBuilderRecoveryStore {
  ...
  /** Logical quota usage, not disk free space, content verification or admission.
   * Includes terminal history. SQL measures BLOB lengths without returning bytes.
   * This snapshot belongs to this store only, not an atomic two-store snapshot. */
  readCapacity() {
    return this.transaction(()=>{
      const operations=(this.db.prepare("SELECT count(*) AS n FROM (SELECT 1 FROM builder_recovery_operations LIMIT 1001)").get() as {n:number}).n;
      const blobs=this.db.prepare(`SELECT count(*) AS files,coalesce(sum(bytes),0) AS bytes,
        coalesce(sum(CASE WHEN kind='blob' AND bytes<=16777216 THEN 0 ELSE 1 END),0) AS invalid
        FROM (SELECT length(content) AS bytes,typeof(content) AS kind FROM builder_recovery_preimages LIMIT 1000001)`).get() as {files:number;bytes:number;invalid:number};
      if(!Number.isSafeInteger(operations)||operations<0||operations>RECOVERY_MAX_OPERATIONS
        ||!Number.isSafeInteger(blobs.files)||blobs.files<operations||blobs.files>operations*1_000
        ||!Number.isSafeInteger(blobs.bytes)||blobs.bytes<0||blobs.bytes>RECOVERY_MAX_STORE_BYTES||blobs.invalid!==0)fail("state-invalid");
      return Object.freeze({kind:"logical-quota-usage-not-admission" as const,
        operationCount:operations,operationLimit:RECOVERY_MAX_OPERATIONS,
        remainingOperationSlots:RECOVERY_MAX_OPERATIONS-operations,
        preimageBytes:blobs.bytes,preimageByteLimit:RECOVERY_MAX_STORE_BYTES,
        remainingPreimageBytes:RECOVERY_MAX_STORE_BYTES-blobs.bytes});
    });
  }
